# spec/services/physical_product_tax_strategy_spec.rb
require 'rails_helper'

RSpec.describe PhysicalProductTaxStrategy do
  ...
  describe '#calculate_tax' do
    context 'when user is from Spain' do
      before do
        ...
        allow(TaxRate).to receive(:find_by).with(country: "Spain", product_type: "good").and_return(tax_rate)
      end
    ...
    context 'when user is from an EU country' do
      ...
      context 'when user is an individual' do
        before do
          ...
          allow(TaxRate).to receive(:find_by).with(country: "France", product_type: "good").and_return(tax_rate)
        end
      ...
      context 'when user is a company' do
        ...
        it 'sets tax_applied to 0 and status to reverse_charge' do
    ...
    context 'when user is from a non-EU country' do
      before do
        ...
        allow(transaction).to receive(:eu_country?).and_return(false)
      end

      it 'sets tax_applied to 0 and status to export' do
        expect(transaction).to receive(:tax_applied=).with(0)
